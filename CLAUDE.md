...
1. **Single control flow** — users write one `src/app.ts`, everything works via async/await
...
- **Fetch proxy** (`runtime/ext-fetch.ts` + `background.ts`): Two-phase fetch—background caches Response, content script reads body via separate RPC call. 60s TTL, single-consume.
